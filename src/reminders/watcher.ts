...
export async function watchRepository(cwd: string, signal: AbortSignal): Promise<void> {
  ...
  while (!signal.aborted) { try { await evaluateRepository(cwd); } catch (error) { if (!signal.aborted) console.error(`CommitCraft watcher: ${error instanceof Error ? error.message : String(error)}`); } await delay(config.reminders.pollIntervalSeconds * 1000, signal); }
}
