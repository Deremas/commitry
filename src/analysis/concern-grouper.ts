...
function buildGroup(name: string, files: ChangedFile[], diff: string, config: CommitryConfig): ConcernGroup { const classification = classifyChanges(files, diff); const scope = name === "project" ? undefined : inferScope(files, config.scopeMappings).scope; const subject = inferSubject(classification.type, files, scope, [], diff); return { name, scope, type: classification.type, files: files.map((file) => file.path), suggestion: `${classification.type}${scope ? `(${scope})` : ""}: ${subject}` }; }
