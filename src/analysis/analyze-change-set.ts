...
export function analyzeChangeSet(files: ChangedFile[], diff: string, branch: string | null, config: CommitCraftConfig, projectTypes: string[] = []): CommitAnalysis {
  ...
  return { type: classification.type, typeConfidence: classification.confidence, scope: scope.scope, scopeConfidence: scope.confidence, subject: inferSubject(classification.type, files, scope.scope, changedSymbols), projectTypes, issueId, breakingChangeCandidate: /BREAKING CHANGE|^[+-].*!:/m.test(diff), mixedConcerns: concernGroups.length > 1, concernGroups, changedSymbols, reasons: [...classification.reasons, ...(scope.reason ? [scope.reason] : []), ...insights.flatMap((item) => item.reasons)] };
}
