...
export async function hookCommand(name: string, cwd = process.cwd()): Promise<void> {
  ...
  console.error(pc.yellow(`Commitry pre-push warning: ${snapshot.files.length} local changed file(s) are not included in this push.`));
