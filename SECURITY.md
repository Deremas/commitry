...
CommitCraft is local-first and does not require network access at runtime. The rule-based provider analyzes local diff content in process. Git operations use argument arrays, and commit messages are passed through a restricted temporary file.
...
Known synthetic fixtures can use an explicit `commitcraft: allow-secret` marker on the same added line. Treat this as a reviewed suppression and never use it to conceal a real credential.
